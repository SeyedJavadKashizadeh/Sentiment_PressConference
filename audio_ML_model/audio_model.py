...
from tf.keras.layers import LSTM, Dense, Activation, Dropout
from tf.keras.optimizers import SGD, RMSprop, Adam, Adadelta, Adagrad, Adamax, Nadam
from tf.keras.models import Sequential
from tf.keras.metrics import Precision, Recall, CategoricalAccuracy
from tf.keras.callbacks import ModelCheckpoint, TensorBoard, History, ReduceLROnPlateau, CSVLogger
...
##    df = df.drop([col for col in df.columns if "mel" in col], axis=1)
    ...
    '''
    y_df = df['emotion']
    counts = [len(df[df.emotion == emotion]) for emotion in df.emotion.unique() if emotion in emotions]
    ...
    for emotion in emotions:
        ...
        x_train = pd.concat([x_train, train_temp.drop(columns=['emotion'])])
        y_train = pd.concat([y_train, pd.DataFrame(train_temp['emotion'])])
        x_test = pd.concat([x_test, test_temp.drop(columns=['emotion'])])
        y_test = pd.concat([y_test, pd.DataFrame(test_temp['emotion'])])
...
def conf_matrix(model, y_test, x_test):
...
infile = 'PATH TO THE CSV FILE CONTAINING EXRACTED FEATURES OF TRAINING AUDIO FILES'
model_path = 'PATH TO FOLDER TO SAVE TRAINED MODELS'
